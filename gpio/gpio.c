...
char *usage = "Usage: gpio -v\n"
              ...
              "       gpio <read/write/aread/awritewb/pwm/mode> ...\n"   /*modify for Roseapple Pi by ELAR Systems*/
		      "       gpio readall/reset\n"
...
static void doExports (int argc, char *argv [])
{
  ...
  for (first = 0, i = 0 ; i < 132 ; ++i)	// Crude, but effective  /*64->32 modify for Roseapple Pi*/
  {
	//add for S500
...
void doUnexportall (char *progName)
{
  ...
  for (pin = 1 ; pin < 132 ; ++pin) /*0->1 63->32 modify for Roseapple Pi*/
  {
...
{
  int model, rev, mem, maker, overVolted, cpumodel ;
...
// Make sure we're on a B+
  ...
  piBoardId (&model, &rev, &mem, &maker, &overVolted, &cpumodel) ;
...
// Version & Warranty
  ...
  if (strcmp (argv [1], "-v") == 0)
  {
    ...
    printf ("Copyright (c) 2015-2017 ELAR Systems\n") ;
    ...
    {
      printf ("Roseapple Pi Details:\n") ;
      printf ("  Type: %s, CPU: %s, Memory: %dMB, Maker: %s %s\n", 
	  piModelNames [model], piCPUNames [cpumodel], mem, piMakerNames [maker], overVolted ? "[OV]" : "") ;
    }
    return 0 ;
  }
