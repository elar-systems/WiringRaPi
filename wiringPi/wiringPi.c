...
// Mask for the bottom 64 pins which belong to the Roseapple Pi
...
/*add for Roseapple Pi by ELAR Systems*/
// for mmap Roseapple Pi 
...
const char *piMakerNames [7] =
{
  ...
  "Qusda",
  "LeMaker",
  "Taiwan", //add for Roseapple Pi by ELAR Systems
} ;

const char *piCPUNames [6] =
{
  "Action S500", //add for Roseapple Pi by ELAR Systems
} ;
...
//pwm for Roseapple Pi only for pwm1
...
void piBoardId (int *model, int *rev, int *mem, int *maker, int *overVolted , int *cpumodel)
{
  FILE *cpuFd ;
  char line [120] ;
  char *c ;
...
static int head2win[8]={11,12,13,15,16,18,22,7}; /*add for Roseapple Pi*/
  ...
  else
  {
	...
	if(BP_REV == version)
	{
		...
		if(edge[bcmGpioPin]==-1)
			return wiringPiFailure (WPI_FATAL, "wiringPiISR: pin not sunpprt on Roseapple Pi (%d,%d)\n", pin,bcmGpioPin) ;
	}
	/*end 2014.08.19*/
...
int wiringPiSetup (void)
{
	...
	else if (BP_REV == boardRev)  /*modify for Roseapple Pi by ELAR Systems*/
	{
		pinToGpio =  pinToGpioR ;
		physToGpio = physToGpioR ;
